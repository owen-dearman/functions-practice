...
/**
 * 
 * @param {object} info - name and type of Pokemon
 * @returns {string} the descriptive statement
 */

function decsribePokemon(info){
    return `${info["name"]} is a ${info["type"]} type Pokemon.`
}
...
function pokemonTypes(pokemonInformation){
    let pokemonDescription = pokemonInformation.map(decsribePokemon);
